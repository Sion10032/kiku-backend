...
import { z } from 'zod';
import { upsertProgress, getWorkProgress, deleteWorkProgress } from '../services/progress.service.js';
...
export const progressRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.put('/progress', {
    ...
    const { work_id, media_index, track_title, position, duration } = request.body;
  ...
  // 删除某作品全部播放进度（回到未读态）
  fastify.delete('/progress/:workId', {
    preHandler: [ fastify.authenticate ],
    schema: {
      params: workIdParamsSchema,
      response: {
