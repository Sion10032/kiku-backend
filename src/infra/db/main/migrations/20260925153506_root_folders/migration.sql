-- 前置条件：本迁移只支持空的 t_work —— 全新安装，或已删除 data/sqlite 后重建。
-- 不支持从旧 kiku 库升级：非空 t_work 会被原样拷贝（SQLite 重建表时 FK 是 OFF 的），
-- 而 t_root_folder 为空 → 留下悬空引用。该降级态不致命且可恢复（重新添加同名根目录即可），
-- 但不属支持路径。见 plans/2026-09-25-root-folders-table.md。
--
-- 本迁移依赖连接处于 foreign_keys=OFF（见 infra/db/main/index.ts：migrate() 先于
-- PRAGMA foreign_keys = ON）。迁移内写 PRAGMA 无效：drizzle migrateSync 把每个迁移包在
-- 事务里，而 SQLite 的 foreign_keys pragma 在事务内是 no-op。
--
...
CREATE TABLE `__new_t_work` (
	...
	CONSTRAINT `fk_t_work_root_folder_t_root_folder_name_fk` FOREIGN KEY (`root_folder`) REFERENCES `t_root_folder`(`name`) ON UPDATE CASCADE ON DELETE RESTRICT,
